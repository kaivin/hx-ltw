    ...
    <div class="page-root">
        ...
        <div class="item-page">
            <div class="item-content" v-if="pageType=='a'">
                ...
                <p>2. <strong>模块分类：</strong>模块自身有其自己的分类，比如：页头、页脚、留言板之类的，都是模块的分类</p>
                <p>3. <strong>适用页面：</strong>模块适合在哪些页面里展现，不同模块类别，可能只适用于部分页面、而不会在所有页面中都适用</p>
                <p>4. <strong>有效宽度：</strong>有固定有效宽度的模块，只适合于和相同有效款宽度的模块放在一起</p>
                <p>5. <strong>添加人：</strong>该模块是有哪位前端人员所贡献的</p>
                <p></p>
                <p>6. <strong>使用类名：</strong>该模块除了唯一标识类名外，其他类名</p>
            ...
            <div class="item-content" v-if="pageType=='b'">
                ...
                <p>5. 站点列表有三个功能：站点预览、站点信息、站点下载；站点预览可以预览该站点的所有页面、站点信息将展示该站点所有页面组成模块的类别、唯一标识类名、有效宽度、添加人等信息；站点下载则可以将该组合站点下载到本地使用</p>
            </div>
            <div class="item-content" v-if="pageType=='c'">
                ...
                <p>4. 提交模块的使用类名，在添加模块时，只写类名即可，不得带"."，多个类名用英文逗号隔开</p>
                <p>5. 模块分类属于页头的会分为侧边固定，以及页头显示两种，对应两种页面框架类型</p>
                <p></p>
                <p>6. 模块分类属于留言板的会分为带联系方式以及不带联系方式两种</p>
                <p>7. 所有模块都会有适用页面，一个模块可以适用于多种页面，也可以只适用于某一种页面</p>
                <p>8. banner模块在提交时，如果只适用于首页的banner有其搭配使用的内页banner，则在提交完只适用于首页banner后，需再提交一个适用内页的模块，该模块选择适用页面选择为除首页之外，此时该模块将拥有一个父级模块唯一标识类名，这里需要填写上一个提交的首页banner的唯一标识类名，如果适用内页的banner，是每个页面都单独适用，则需要针对每个页面分别提交一个适用于该内页的banner模块，其父级模块唯一标识类名，则同属于一个</p>
                <p>9. 组合页头模块与banner模块的提交原理相同</p>
                <p>10. 图文、文字、组合列表如果只适用于首页，则适用页面只选择首页即可，只适用内页原理相同</p>
                <p></p>
                ...
                <p>15. 针对静态图片资源会出现不同模块不同图片出现相同命名，自行注意图片命名，可自己定义一个命名规则，防止这种情况发生，针对不同前端人员可能出现相同命名的情况，图片统一加前缀名字缩写</p>
                <p></p>
                <p>16. 针对静态图片资源会出现不同模块相同图片出现不同命名，从而产生不必要的多重资源问题，需尽量做到沿用之前模块的图片资源，也就是说，允许当前模块引用之前模块的图片资源，命名可以保持为之前模块的图片资源命名</p>
                <p>17. 模块本身的样式中禁止使用以及设置字体，字体一律会在公共样式中设置</p>
                <p>18. 模块添加完成后，可以在本系统的模块页面通过设置搜索条件查看模块，搜索结果的模块展示列表中，模块拥有编辑、删除、复制代码功能</p>
                <p>19. 侧边导航固定框架类型的只需提交框架内的代码，整个页面大框架代码则由本系统提供一套固定的代码</p>
                <p>20. 在该系统中不存在左右布局类型框架，所有左右布局的模块统一写成一个大模块，在模块分类中凡是带组合字样的，都属于多模块组合而成，不可拆分的模块</p>
                <p></p>
                <p>21. 代码和样式中禁止使用单行注释，有需要用到引号的，尽量都使用双引号，避免使用单引号</p>
                <p>22. <strong>不同模块如果出现使用相同类名的情况，该类名不可为最外层级，防止样式冲突，模块所有样式均需通过模块唯一标识类名进行控制</strong></p>
                <p>23. <strong>禁止出现和标签相同的类名，以及与js关键字相同的类名、图片命名中禁止出现包含类名的命名</strong></p>
                <p>24. 所有模块的文字中，凡是表示公司名的，一律使用 $company$ 替代，在站点下载操作时，会弹窗让输入该站点公司名，公司名需先预览该站点，看logo是否包含公司名，如logo包含公司名，则输入的公司名即logo代表的公司名，如不包含，则公司名随意定即可</p>
            </div>
            ...
            <div class="item-content" v-if="pageType=='e'">
                ...
                <p>3. 站点的有效宽度必须先行设置，固定的 1440 1280 960，这三个有效宽度彼此是不能在同一个页面出现的，但他们都可以和通屏这个有效宽度进行组合，所以有效宽度的选择最多为两项，最少一项</p>
                ...
                <p></p>
                <p>6. 页面由不同页面模块组合，该页面模块根据展现内容不同而有所分类，而有些页面模块所对应的模块则可以是多个，在选择了这类页面模块之后，会出现需要选择对应模块的选项，这些也需要做选择，选择几项，则该页面模块最终的数据抽取就从所选择的模块数据中进行抽取</p>
                ...
                <p></p>
                <p>11. 站点下载则可以将该站点的所有资源的压缩包下载到本地，资源中包括已经生成的各个html页面，css样式文件、该站点所有页面及样式中应用到的静态图片资源、静态JQ库文件以及站点信息文本文件</p>
                <p>12. 站点下载后需要本地进行调试，调试内容包括但不限于：文案修改统一、色调统一、模块微调、模块替换，当然此时如果想使用特殊字体，将不再做限制</p>
                <p>13. 站点下载后，会出现可能该组合某个模块并不理想，需要替换，此时可以在本系统的模块页面中根据设置搜索条件，搜索出需要的模块，模块页面展示的模块除了编辑、删除功能外，会有复制HTML代码以及复制CSS代码的功能，找到需要的模块复制模块的HTML代码和CSS代码替换需要替换的模块即可，而该模块所引用的图片资源，需要通过html和css代码中的引用自行去找到并复制到站点的图片资源文件夹中</p>
